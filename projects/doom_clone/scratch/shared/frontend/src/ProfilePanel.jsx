...
export default function ProfilePanel({ onBootstrapped }) {
  ...
  // Listen for external updates to refresh profile panel
  useEffect(() => {
    const onUpdated = (e) => {
      const p = e && e.detail;
      if (!p) return;
  ...
  // Also refresh on hydration event (spawned by app shell)
  useEffect(() => {
    const onHydrated = (ev) => {
    ...
    window.addEventListener('doom-app-profile-hydrated', onHydrated);
    return () => window.removeEventListener('doom-app-profile-hydrated', onHydrated);
  }, []);
